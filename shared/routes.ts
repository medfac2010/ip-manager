import { z } from 'zod';
import { insertEstablishmentSchema, insertUserSchema, insertPcSchema, establishments, users, pcs } from './schema';

export * from "./schema";

export const errorSchemas = {
  validation: z.object({
    message: z.string(),
    field: z.string().optional(),
  }),
  notFound: z.object({
    message: z.string(),
  }),
  internal: z.object({
    message: z.string(),
  }),
  unauthorized: z.object({
    message: z.string(),
  }),
};

export const api = {
  auth: {
    login: {
      method: 'POST' as const,
      path: '/api/login',
      input: z.object({ username: z.string(), password: z.string() }),
      responses: {
        200: z.custom<typeof users.$inferSelect>(),
        401: errorSchemas.unauthorized,
      },
    },
    logout: {
      method: 'POST' as const,
      path: '/api/logout',
      responses: {
        200: z.object({ message: z.string() }),
      },
    },
    me: {
      method: 'GET' as const,
      path: '/api/user',
      responses: {
        200: z.custom<typeof users.$inferSelect>(),
        401: errorSchemas.unauthorized,
      },
    },
    changePassword: {
      method: 'POST' as const,
      path: '/api/change-password',
      input: z.object({ currentPassword: z.string(), newPassword: z.string() }),
      responses: {
        200: z.object({ message: z.string() }),
        400: errorSchemas.validation,
      },
    },
  },
  establishments: {
    list: {
      method: 'GET' as const,
      path: '/api/establishments',
      responses: {
        200: z.array(z.custom<typeof establishments.$inferSelect>()),
      },
    },
    create: {
      method: 'POST' as const,
      path: '/api/establishments',
      input: insertEstablishmentSchema,
      responses: {
        201: z.custom<typeof establishments.$inferSelect>(),
      },
    },
    update: {
      method: 'PUT' as const,
      path: '/api/establishments/:id',
      input: insertEstablishmentSchema.partial(),
      responses: {
        200: z.custom<typeof establishments.$inferSelect>(),
        404: errorSchemas.notFound,
      },
    },
    delete: {
      method: 'DELETE' as const,
      path: '/api/establishments/:id',
      responses: {
        204: z.void(),
      },
    },
  },
  users: {
    create: {
      method: 'POST' as const,
      path: '/api/users',
      input: insertUserSchema,
      responses: {
        201: z.custom<typeof users.$inferSelect>(),
        400: errorSchemas.validation,
      },
    },
    list: {
      method: 'GET' as const,
      path: '/api/users',
      responses: {
        200: z.array(z.custom<typeof users.$inferSelect>()),
      },
    },
    update: {
      method: 'PUT' as const,
      path: '/api/users/:id',
      input: insertUserSchema.partial(),
      responses: {
        200: z.custom<typeof users.$inferSelect>(),
        404: errorSchemas.notFound,
      },
    },
    delete: {
      method: 'DELETE' as const,
      path: '/api/users/:id',
      responses: {
        204: z.void(),
      },
    },
  },
  pcs: {
    list: {
      method: 'GET' as const,
      path: '/api/pcs',
      input: z.object({ establishmentId: z.string().optional() }).optional(),
      responses: {
        200: z.array(z.custom<typeof pcs.$inferSelect>()),
      },
    },
    create: {
      method: 'POST' as const,
      path: '/api/pcs',
      input: insertPcSchema,
      responses: {
        201: z.custom<typeof pcs.$inferSelect>(),
        400: errorSchemas.validation,
      },
    },
    update: {
      method: 'PUT' as const,
      path: '/api/pcs/:id',
      input: insertPcSchema.partial(),
      responses: {
        200: z.custom<typeof pcs.$inferSelect>(),
        404: errorSchemas.notFound,
      },
    },
    delete: {
      method: 'DELETE' as const,
      path: '/api/pcs/:id',
      responses: {
        204: z.void(),
      },
    },
  },
  stats: {
    get: {
      method: 'GET' as const,
      path: '/api/stats',
      responses: {
        200: z.array(z.object({
          establishmentId: z.number(),
          establishmentName: z.string(),
          totalPcs: z.number(),
          protectedPcs: z.number(),
          windowsLicensed: z.number(),
          officeLicensed: z.number(),
          serverCount: z.number(),
          filteredIpCount: z.number(),
        })),
      },
    },
  },
};

export function buildUrl(path: string, params?: Record<string, string | number>): string {
  let url = path;
  if (params) {
    Object.entries(params).forEach(([key, value]) => {
      if (url.includes(`:${key}`)) {
        url = url.replace(`:${key}`, String(value));
      }
    });
  }
  return url;
}
